...
class Simulator(ancestry.SuperSimulator):
    ...
    def __post_init__(self):
        ...
        self.mean_load = self.U / self.s
        self.Q = self.generate_q()
        self.num_fitness_classes = self.Q.shape[0]
        self.num_lineages = np.zeros(self.num_fitness_classes)        
        self.idx_offset = self.mean_load - self.num_fitness_classes//2
        self.lineages = []
    ...
    def remove_lineage(self, lineage_id):
        ...
        idx = lin.value - self.idx_offset
        self.num_lineages[idx] -= 1
        return lin

    def insert_lineage(self, lineage):
        self.lineages.append(lineage)
        idx = lineage.value - self.idx_offset
        self.num_lineages[idx] += 1
    ...
    def run(self, simplify=True):
        return self.model._sim(simplify)
    ...
    def _sim(self, simplify=True):
        ...
        while not self.stop_condition():
            ...
            if t_inc == t_re:  # recombination
                ...
                right_lineage = left_lineage.split(breakpoint, self.K)
                ...
                idxs = rng.choice(
                    np.arange(np.sum(self.num_lineages)),
                    size=2, 
